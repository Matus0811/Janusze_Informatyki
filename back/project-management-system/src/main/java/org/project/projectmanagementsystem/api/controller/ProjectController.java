...
public class ProjectController {
    ...
    public ResponseEntity<?> addUsersToProject(@PathVariable("projectId") String projectId, @RequestBody AssignFormDTO assignFormDTO) {
        AssignForm assignForm = FormMapper.INSTANCE.mapFromDtoToDomain(assignFormDTO);
        assignForm = assignForm.withProjectId(projectId);

        projectService.addUsersToProject(assignForm);
        return new ResponseEntity<>(HttpStatus.ACCEPTED);
    }

    @GetMapping("/{projectId}/unassigned-users")
    public ResponseEntity<List<UserDTO>> getUnassignedUsers(@PathVariable(name = "projectId") String projectId){
